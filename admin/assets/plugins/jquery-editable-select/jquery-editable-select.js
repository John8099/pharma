+(function ($) {
  ...
  EditableSelect.prototype.show = function () {
    ...
    if (
      ...
      this.$list[fn](
        ...
        $.proxy(this.utility.trigger, this.utility, "shown")
      );
    }
  };
  EditableSelect.prototype.hide = function () {
    ...
    this.$list[fn](
      this.options.duration,
      $.proxy(this.utility.trigger, this.utility, "hidden")
    );
  };
  ...
  EditableSelectUtility.prototype.initializeInput = function () {
    ...
    switch (this.es.options.trigger) {
      default:
      case "focus":
        that.es.$input.on("focus", $.proxy(that.es.show, that.es)).on(
          "blur",
          $.proxy(function () {
    ...
    setTimeout(function () {
      ...
      if (visibles.length > 0) {
        ...
        if (
          ...
          top + selected.outerHeight() > that.es.$list.outerHeight()
        )
          that.es.$list.scrollTop(
            that.es.$list.scrollTop() +
              selected.outerHeight() +
              2 * (top - that.es.$list.outerHeight())
          );
      }
    });
  };
  EditableSelectUtility.prototype.setAttributes = function (
    $element,
    attrs,
    data
  ) {
